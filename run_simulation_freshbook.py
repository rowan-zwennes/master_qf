"""Fork of run_simulation.py carrying the publication-clock placement arm.

Kept as a separate file rather than a flag so the headline results stay
reproducible from an engine this arm never touched. The two files are
therefore near-identical by design; the placement clock is the difference."""
...
class SimConfig:
    ...
    place_on_book_update: bool = False
    chi0: float = 0.0
    chi1: float = 0.0
...
class DayArrays:
    ...
    last_event_time: np.ndarray  # (n,) i64 ms
...
def simulate_day(
    ...
    defer = bool(cfg.place_on_book_update)
    pending: dict[int, tuple] = {}
    if defer:
        let = np.asarray(day.last_event_time, dtype=np.int64)
        pub_new = np.zeros(n, dtype=bool)
        pub_new[1:] = (let[1:] > let[:-1]) & (let[1:] > 0)
        if n > 1 and not pub_new.any():
            raise ValueError(
                f"{day.fday}: last_event_time yields no publication instants; "
                f"the placement arm would never submit a quote")

    def drain_trades(bound: int, i: int) -> None:
        """Process trades strictly before `bound` on the true event clock,
        with settlements interleaved at their exact millisecond. Split out of
        the tick loop so the placement arm can interleave a submission at the
        publication instant; called once per tick when the arm is off, so the
        behaviour is unchanged."""
        nonlocal trade_cursor, settle_i, pause_i
        while trade_cursor < day.trade_ts.size \
                and day.trade_ts[trade_cursor] < bound:
    ...
    def depths_for(st: StratState, ctx: tuple, q_int: int) -> tuple[float, float]:
        """The quote depths of the tick loop, evaluated for an arbitrary inventory."""
        alpha_c, u_c, r1_use, force_r2_c, _ = ctx
        spec = st.spec
        if chi_on:
            db_t, da_t = quote_depths(spec, mc_chi, q_int, 0.0, u_c, None,
                                      fixed_half_spread=cfg.fixed_half_spread,
                                      r1=r1_chi)
            db = chi_unscale(db_t, chi0_eff, cfg.chi1)
            da = chi_unscale(da_t, chi0_eff, cfg.chi1)
        else:
            db, da = quote_depths(spec, mc, q_int, alpha_c, u_c, st.r2,
                                  fixed_half_spread=cfg.fixed_half_spread,
                                  r1=r1_use, force_r2=force_r2_c,
                                  drift_in_lut=cfg.ml_bake_drift_in_lut)
        if alpha_c != 0.0 and cfg.ml_shift_mode == "defensive":
            db0, da0 = quote_depths(spec, mc, q_int, 0.0, u_c, st.r2,
                                    fixed_half_spread=cfg.fixed_half_spread,
                                    r1=r1_auto, force_r2=force_r2_c)
            db, da = max(db, db0), max(da, da0)
        return db, da

    def submit(st: StratState, i: int, ts: int, mid_a: float, bid0_a: float,
               ask0_a: float, db: float, da: float, ml_alpha: float) -> None:
        """Desired prices: tick-rounded away from the anchor, post-only
        clamped, floored at delta_min, suppressed at the inventory cap; then
        the keep-or-replace decision against whatever is resting. Split out of
        the tick loop verbatim so the placement arm can call it at a
        publication instant with the anchor and book of that instant."""
        want_bid = (st.inv + qs <= Q * qs + 1e-9) and math.isfinite(db)
        want_ask = (st.inv - qs >= -Q * qs - 1e-9) and math.isfinite(da)
        bpx = apx = math.nan
        if want_bid:
            bpx = _round_down(mid_a - max(db, cfg.delta_min), tick)
            bpx = _round_down(min(bpx, ask0_a - tick), tick)
            if bpx <= 0.0:
                want_bid = False
        if want_ask:
            apx = _round_up(mid_a + max(da, cfg.delta_min), tick)
            apx = _round_up(max(apx, bid0_a + tick), tick)
        ...
        if want_bid and st.bid_id < 0:
            ...
            st.bid_id = st.tracker.place(+1, bpx, qs, qa, ts)
            st.bid_px = bpx
        ...
        if want_ask and st.ask_id < 0:
            ...
            st.ask_id = st.tracker.place(-1, apx, qs, qa, ts)
            st.ask_px = apx
            st.ask_alpha_ml = ml_alpha

    def place_pending(i: int, p: int) -> None:
        """Submit the quotes decided at the last grid tick, at publication
        instant p, against row i's book (which is the book as of p) and the
        inventory we hold at p."""
        nonlocal pause_i
        if not bool(day.valid[i]) or p < warmup_until:
            return
        while pause_i < len(pauses) and p >= pauses[pause_i][1]:
            pause_i += 1
        if pause_i < len(pauses) and pauses[pause_i][0] <= p:
            return
        mid_a = float(ta.mid[i])
        bid0_a = float(day.bid_p[i, 0])
        ask0_a = float(day.ask_p[i, 0])
        if not (bid0_a > 0.0 and ask0_a > bid0_a):
            return
        for sid, st in states.items():
            ctx = pending.get(sid)
            if ctx is None:
                continue
            db, da = depths_for(st, ctx, st.q_lots(qs, Q))
            submit(st, i, p, mid_a, bid0_a, ask0_a, db, da, ctx[4])

    for i in range(n):
        t = int(day.tick_ts[i])
        place_now = False
        if defer and pub_new[i]:
            p_i = int(let[i])
            if p_i == t:
                place_now = True
            elif pending and int(day.tick_ts[i - 1]) < p_i < t:
                drain_trades(p_i + 1, i)
                place_pending(i, p_i)
        ...
        for sid, st in states.items():
            ...
            if not defer or place_now:
                submit(st, i, t, mid, bid0, ask0, db, da, ml_alpha)
                if defer:
                    pending.pop(sid, None)
            else:
                pending[sid] = (alpha, u, r1_use, force_r2, ml_alpha)
...
def load_day_arrays(base: Path, fday: str, cfg: SimConfig,
    ...
    cols = (["ts_ms", "valid", "last_event_time"]
    ...
    last_event_time = (book["last_event_time"].fill_null(0)
...
def _make_synthetic_day(seed: int = 7, minutes: float = 12.0) -> DayArrays:
    ...
    pub = t0 + 102 * np.arange(n + 2, dtype=np.int64)
    last_event_time = pub[np.searchsorted(pub, tick_ts, side="right") - 1]
...
def main() -> None:
    ...
    ap.add_argument("--place-on-book-update", action="store_true",
                    help="publication-clock arm: decide on the 100 ms grid, "
                         "but submit at the next venue publication, anchored "
                         "to the book and inventory of that instant")
    ...
    cfg = SimConfig(
        ...
        place_on_book_update=args.place_on_book_update,
